...
def test_queryEndpoint_returnsGeneratedCommand(mockGenerateCommand):
    ...
    response = client.post("/query", json={"query": "list files", "context": MOCK_CONTEXT})
...
def test_queryEndpoint_forwardsQueryAndContextToGenerator(mockGenerateCommand):
    """Verify the endpoint passes both the query and context through to the generator unchanged."""
    mockGenerateCommand.return_value = "uptime"

    client.post("/query", json={"query": "show system uptime", "context": MOCK_CONTEXT})

    mockGenerateCommand.assert_called_once_with("show system uptime", MOCK_CONTEXT)
...
def test_queryEndpoint_returns422WhenValidatorRejects(mockGenerateCommand):
    ...
    response = client.post("/query", json={"query": "delete everything", "context": MOCK_CONTEXT})
